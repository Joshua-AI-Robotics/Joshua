  ...
  OperationalLimitCalibration(const std::string& node_name,
    ...
    for (const auto& single_calibration : config.calibration().single_calibrations()) {
      ...
      for (int i = 0; i < node.subscriptions_size(); ++i) {
        ...
        auto callback = [this,
                         &operational_limit](const std_msgs::msg::UInt8MultiArray::SharedPtr msg) {
          auto parsed = ros2_utils::ParsePerceptionPacket(msg->data);
          if (!parsed.ok()) {
            RCLCPP_WARN(this->get_logger(),
                        "Failed to parse PerceptionPacket on topic '%s': %s",
                        operational_limit.subscribe_topic.c_str(),
                        parsed.status().message().data());
            return;
          }

          auto position = ros2_utils::RequirePerceptionPosition(parsed.value());
          if (!position.ok()) {
            RCLCPP_WARN(this->get_logger(),
                        "Failed to read position from PerceptionPacket on topic '%s': %s",
                        operational_limit.subscribe_topic.c_str(),
                        position.status().message().data());
            return;
          }

          const float value = position.value();
        ...
        operational_limit.subscription = this->create_subscription<std_msgs::msg::UInt8MultiArray>(
