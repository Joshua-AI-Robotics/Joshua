...
robot::perception::SinglePerception MakeSensorConfig() {
  robot::perception::SinglePerception perception;
  perception.set_sensor_name("joint_1");
  perception.set_id(1);
  perception.set_sensor_type(robot::perception::SensorType::POSITION);
  perception.mutable_sts3215_encoder_config()->set_board_name("arm_bus");
  perception.mutable_sts3215_encoder_config()->set_channel(1);
  return perception;
}

TEST(Sts3215PositionSensorTest, PublishesThePositionTheChannelReports) {
...
TEST(Sts3215PositionSensorTest, InitDoesNotTouchTheBus) {
  auto channel = std::make_shared<RecordingChannel>(robot::board::ChannelFeedback{});
  Sts3215PositionSensor sensor(channel, MakeSensorConfig());
...
TEST(Sts3215PositionSensorTest, TeardownLeavesTheBoardAlone) {
  auto channel = std::make_shared<RecordingChannel>(robot::board::ChannelFeedback{});
  Sts3215PositionSensor sensor(channel, MakeSensorConfig());
...
TEST(Sts3215PositionSensorTest, PropagatesReadFailures) {
  auto channel = std::make_shared<RecordingChannel>(robot::board::ChannelFeedback{});
