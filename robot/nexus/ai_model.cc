...
AIModel::AIModel() : pybind_data_(std::make_unique<PybindData>()) {
    ...
    LOG(INFO) << "AIModel constructor completed, GIL released, thread ID: " << std::this_thread::get_id();
}
...
bool AIModel::Init(const std::string& module_name, const std::string& function_name) {
    ...
    try {
        LOG(INFO) << "About to acquire GIL in Init()";
        py::gil_scoped_acquire acquire;
        LOG(INFO) << "GIL acquired successfully in Init()";
...
NexusModelOutputPacket AIModel::Predict(const NexusModelInputPacket& input_packet) {
    LOG(INFO) << "DEBUG: AIModel::Predict called on thread: " << std::this_thread::get_id();
    ...
    try {
        LOG(INFO) << "DEBUG: About to acquire GIL in Predict()...";
        ...
        LOG(INFO) << "DEBUG: GIL acquired successfully in Predict() after " << duration.count() << "ms";
        
        LOG(INFO) << "DEBUG: Importing module...";
        ...
        LOG(INFO) << "DEBUG: GIL will be released automatically";
    } catch (py::error_already_set &e) {
