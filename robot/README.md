...
- **Board leg.** The device multiplexes several channels over one link (a
  ...
  gets a `SensorChannel`, so a sensor and an actuator on one bus share a board
...
- `board/` — `interfaces/`: `ActuatorChannel` (command) and `SensorChannel`
  (measurement) are separate seams, and `BoardChannel` is their union for a
  slot that does both, so a consumer takes only the half it needs and a
  sensor holds nothing that can move a motor. Plus `BoardInterface`, and
  `factory/` with its per-board instance cache, the shared channel resolver,
  and the motor/drive and sensor/signal compatibility tables,
...
- `perception/` — sensor drivers behind the single `PerceptionInterface`
  ...
  `SensorChannel`, owning no port), `camera/` and `lidar/` for single-stream
