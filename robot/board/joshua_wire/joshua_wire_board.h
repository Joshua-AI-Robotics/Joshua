...
// Joshua-firmware MCU board (TeensyBoard, ArduinoBoard, Esp32Board today,
// ...) speaks the exact same wire protocol, so this class holds that
// entire orchestration once — a concrete board subclasses this
...
// needs to differ: every board on this class (Teensy, Arduino, ESP32) is a
// serial FrameTransport speaking STEP_DIR channels, so ValidateComm and
// CreateTransport below default to exactly that. ArduinoBoard and
// Esp32Board override CreateTransport only to wait out a post-open board
// reset (the Uno's DTR auto-reset, the ESP32 bridge's EN auto-reset)
// before the first frame; the handshake itself is unchanged.
//
