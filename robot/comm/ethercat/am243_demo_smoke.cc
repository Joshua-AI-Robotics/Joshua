...
using robot::action::am243::EncodeDemoOutputWalk;
...
int main(int argc, char** argv) {
  ...
  for (int cycle = 0; cycle < cycles; ++cycle) {
    ...
    const std::vector<uint8_t> outputs = EncodeDemoOutputWalk(seed);
    ...
    std::cout << "cycle=" << cycle << " seed=" << static_cast<int>(seed)
              << " echo=" << static_cast<int>(*echo_or) << " wkc=" << process_data_or->working_count
