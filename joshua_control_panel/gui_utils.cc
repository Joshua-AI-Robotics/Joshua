...
Handle show(QWidget* parent, const QString& title, const QStringList& steps) {
    Handle handle;
    QMetaObject::invokeMethod(ensureManager(), [parent, title, steps, &handle]() {
...
void markStepDone(const Handle& handle, int stepIndex) {
    ...
    QMetaObject::invokeMethod(ensureManager(), [id = handle.id, stepIndex]() {
        ensureManager()->markStepDone(id, stepIndex);
    }, Qt::QueuedConnection);
}

void setFinalMessage(const Handle& handle, const QString& finalLine) {
    if (handle.id == 0) return;
    QMetaObject::invokeMethod(ensureManager(), [id = handle.id, finalLine]() {
        ensureManager()->setFinalMessage(id, finalLine);
    }, Qt::QueuedConnection);
}

void close(const Handle& handle) {
    if (handle.id == 0) return;
    QMetaObject::invokeMethod(ensureManager(), [id = handle.id]() {
        ensureManager()->close(id);
    }, Qt::QueuedConnection);
}
