...
constexpr auto kErrorStyle =
    "QLabel { background-color: #ff0000; padding: 5px; border-radius: 5px; }";
constexpr auto kRunningStyle =
    "QLabel { background-color: #4caf50; padding: 5px; border-radius: 5px; }";
constexpr auto kStoppingStyle =
    "QLabel { background-color: #ffa500; padding: 5px; border-radius: 5px; }";
constexpr auto kIdleStyle =
    "QLabel { background-color: #cccccc; color:rgb(0, 0, 0); padding: 5px; border-radius: 5px; }";
...
MonitorTab::MonitorTab(QWidget* parent) : QWidget(parent), ui(new Ui::MonitorTab) {
  ...
  connect(ui->config_preset_listView->selectionModel(),
          &QItemSelectionModel::currentChanged,
          this,
          &MonitorTab::onConfigPresetSelectionChanged);
  ...
  connect(this,
          &MonitorTab::setLaunchButtonEnabled,
          this,
          &MonitorTab::onSetLaunchButtonEnabled,
          Qt::QueuedConnection);
  connect(this,
          &MonitorTab::updateNodeTable,
          this,
          &MonitorTab::onUpdateNodeTable,
          Qt::QueuedConnection);
...
void MonitorTab::onUpdateNodeTable() {
  ...
  for (size_t i = 0; i < launched_nodes.size(); ++i) {
    ...
    ui->nodeTable->setItem(
        i, 2, new QTableWidgetItem(QString::fromStdString(std::to_string(node.node_id))));
    ui->nodeTable->setItem(
        i, 3, new QTableWidgetItem(QString::number(static_cast<qlonglong>(node.pid))));
  }
...
void MonitorTab::onConfigPresetSelectionChanged(const QModelIndex& current,
...
bool MonitorTab::setup_node_generator() {
  try {
    ...
    emit logMessage("[INFO] Initializing node generator with config: " +
    ...
    onLogMessage("[ERROR] Exception during node generator setup: " +
