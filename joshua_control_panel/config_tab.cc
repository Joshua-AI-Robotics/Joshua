...
void ConfigTab::on_loadButton_clicked() {
  onLoadPbtxt();
}
void ConfigTab::on_clearButton_clicked() {
  onClear();
}
...
std::unique_ptr<ConfigTab::MessageNode> ConfigTab::buildMessageNode(const Descriptor* descriptor,
                                                                    const QString& title) {
  auto node = std::make_unique<MessageNode>();
  node->descriptor = descriptor;

  node->group = new QGroupBox(
      title.isEmpty() ? QString::fromStdString(descriptor->full_name()) : title, this);
  ...
  for (int i = 0; i < descriptor->field_count(); ++i) {
    ...
    if (field->is_repeated() && field->cpp_type() == FieldDescriptor::CPPTYPE_MESSAGE) {
      ...
      QObject::connect(
          repPtr->addButton, &QPushButton::clicked, container, [this, field, repPtr]() {
            auto itemNode = buildMessageNode(field->message_type(),
                                             QString::fromStdString(field->message_type()->name()));
  ...
  for (int i = 0; i < descriptor->oneof_decl_count(); ++i) {
    ...
    QObject::connect(oneNode.selector,
                     qOverload<int>(&QComboBox::currentIndexChanged),
                     container,
                     [stacked = oneNode.stacked](int index) {
...
void ConfigTab::addRepeatedPrimitiveRow(RepeatedPrimitiveFieldNode* node, const QVariant& initial) {
  ...
  QObject::connect(removeBtn, &QPushButton::clicked, row, [node, row]() {
    ...
    auto it = std::find_if(
        node->items.begin(),
        node->items.end(),
        [row](const RepeatedPrimitiveFieldNode::RowItem& it) { return it.rowWidget == row; });
...
std::unique_ptr<ConfigTab::MessageNode> ConfigTab::buildSingleMessageField(
    const FieldDescriptor* field) {
  auto child = buildMessageNode(field->message_type(), QString::fromStdString(field->name()));
  return child;
}

ConfigTab::RepeatedMessageFieldNode ConfigTab::buildRepeatedMessageField(
    const FieldDescriptor* field) {
...
ConfigTab::RepeatedPrimitiveFieldNode ConfigTab::buildRepeatedPrimitiveField(
    const FieldDescriptor* field) {
...
void ConfigTab::onSavePbtxt() {
  ...
  QString filename =
      QFileDialog::getSaveFileName(this, "Save configuration", QString(), "Prototxt (*.pbtxt)");
...
void ConfigTab::onLoadPbtxt() {
  if (!rootNode_) return;
  QString filename =
      QFileDialog::getOpenFileName(this, "Load configuration", QString(), "Prototxt (*.pbtxt)");
  if (filename.isEmpty()) return;
...
void ConfigTab::writeRepeatedPrimitiveField(const RepeatedPrimitiveFieldNode& node,
                                            Message* message) {
  const Reflection* refl = message->GetReflection();
  const FieldDescriptor* field = node.field;
...
void ConfigTab::readRepeatedMessageFieldIntoChildren(const Message& message,
                                                     RepeatedMessageFieldNode* node) {
  const Reflection* refl = message.GetReflection();
  const FieldDescriptor* field = node->field;
  ...
  for (int i = 0; i < count; ++i) {
    auto child = buildMessageNode(field->message_type(),
                                  QString::fromStdString(field->message_type()->name()));
...
void ConfigTab::readRepeatedPrimitiveFieldIntoEditors(const Message& message,
                                                      RepeatedPrimitiveFieldNode* node) {
  const Reflection* refl = message.GetReflection();
  const FieldDescriptor* field = node->field;
  ...
  for (int i = 0; i < count; ++i) {
    ...
    switch (field->type()) {
      case FieldDescriptor::TYPE_STRING:
        initial = QString::fromStdString(refl->GetRepeatedString(message, field, i));
        break;
      case FieldDescriptor::TYPE_BOOL:
        initial = refl->GetRepeatedBool(message, field, i);
        break;
      case FieldDescriptor::TYPE_INT32:
        initial = refl->GetRepeatedInt32(message, field, i);
        break;
      case FieldDescriptor::TYPE_UINT32:
        initial = static_cast<int>(refl->GetRepeatedUInt32(message, field, i));
        break;
      case FieldDescriptor::TYPE_INT64:
        initial = QString::number(refl->GetRepeatedInt64(message, field, i));
        break;
      case FieldDescriptor::TYPE_UINT64:
        initial = QString::number(refl->GetRepeatedUInt64(message, field, i));
        break;
      case FieldDescriptor::TYPE_FLOAT:
        initial = static_cast<double>(refl->GetRepeatedFloat(message, field, i));
        break;
      case FieldDescriptor::TYPE_DOUBLE:
        initial = refl->GetRepeatedDouble(message, field, i);
        break;
      default:
        break;
    }
