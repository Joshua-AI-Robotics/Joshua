...
def _detect_gait_cycle(positions: np.ndarray) -> int | None:
    """Find gait period via autocorrelation on the first joint's position."""
...
def _write_trajectory_pbtxt(
    ...
    """Write a .pbtxt trajectories block matching TrajectoryPublisher format."""
    lines = [
        "trajectories {",
        "  single_trajectories {",
        "    node {",
...
def _export_trajectory_data(
    ...
    detect_cycle = cfg.get("detect_cycle", False)
    ...
    if detect_cycle:
        period = _detect_gait_cycle(pos_data)
...
def _trajectory_export_rsl_rl(cfg: dict) -> None:
    ...
    env.close()
    _export_trajectory_data(
        all_positions, all_actions, step_dt, cfg, joint_names)


def _trajectory_export_skrl(cfg: dict) -> None:
...
def _run_trajectory_export(cfg: dict) -> None:
    ...
    if algorithm == "rsl_rl":
        _trajectory_export_rsl_rl(cfg)
    elif algorithm == "skrl":
        _trajectory_export_skrl(cfg)
    else:
        raise ValueError(
