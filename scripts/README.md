...
A Pybricks (MicroPython) program that runs **on a LEGO SPIKE Prime hub** and bridges line-based `SET`/`GET` motor commands over stdin/stdout. Deploy it via the Pybricks app or `pybricksdev`. The host-side Python driver that
drove it was removed in [BOARD_LAYER_RFC.md](../docs/BOARD_LAYER_RFC.md) §10 Phase 9;
this script is kept for manual hub-side testing until the native `SPIKE_HUB_BLE`
board lands.
