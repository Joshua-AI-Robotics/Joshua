...
def replace_known(text: str, replacements: dict[str, str]) -> tuple[str, list[tuple[str, str]], list[str]]:
    updated = text
...
parser = ArgumentParser(description="Patch Industrial Communications SDK 09 imports.mak for local TI tool paths")
parser.add_argument(
    "--sdk-root",
    default=str(Path.home() / "ti" / "ind_comms_sdk_am243x_09_00_00_03"),
    help="Path to Industrial Communications SDK 09 root",
)
parser.add_argument("--tools-path", default=str(Path.home() / "ti"), help="Path to TI tools root")
parser.add_argument("--ccs-dir", default="ccs2100/ccs", help="Relative CCS directory under tools path")
parser.add_argument("--sysconfig-dir", default="sysconfig_1.17.0", help="Relative SysConfig directory under tools path")
parser.add_argument(
    "--compiler-dir",
    default="ccs2100/ccs/tools/compiler/ti-cgt-armllvm_5.1.1.LTS",
    help="TI ARM Clang directory under tools path",
)
parser.add_argument("--pru-compiler-dir", default="ti-cgt-pru_2.3.3/ti-cgt-pru_2.3.3", help="TI PRU compiler directory under tools path")
...
for imports_file in imports_files:
    ...
    replacements = {
        "export CCS_PATH?=$(TOOLS_PATH)/ccs1240/ccs": f"export CCS_PATH?=$(TOOLS_PATH)/{args.ccs_dir}",
        "SYSCFG_PATH ?= $(TOOLS_PATH)/sysconfig_1.17.0": f"SYSCFG_PATH ?= $(TOOLS_PATH)/{args.sysconfig_dir}",
