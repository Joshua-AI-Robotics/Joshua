...
parser = ArgumentParser(
    description="Patch TI MCU+ SDK imports.mak for AM243 build environment"
)
parser.add_argument(
    "--sdk-root",
    default=str(Path.home() / "ti" / "mcu_plus_sdk_am243x_12_00_00_26"),
    help="Path to the MCU+ SDK root",
)
parser.add_argument(
    "--ccs-dir",
    default="ccs2100/ccs",
    help="Relative CCS installation directory under TOOLS_PATH",
)
parser.add_argument(
    "--sysconfig-dir",
    default="sysconfig_1.26.3",
    help="Relative SysConfig installation directory under TOOLS_PATH",
)
parser.add_argument(
    "--compiler-dir",
    default="ti-cgt-armllvm_5.1.1.LTS",
    help="Compiler directory name under CCS_PATH/tools/compiler",
)
parser.add_argument(
    "--tools-path",
    default=str(Path.home() / "ti"),
    help="Path to the TOOLS_PATH used by imports.mak",
)
parser.add_argument(
    "--dry-run",
    action="store_true",
...
if missing:
    print(
        f"Skipped {len(missing)} known replacement(s) because source text was not found"
    )
